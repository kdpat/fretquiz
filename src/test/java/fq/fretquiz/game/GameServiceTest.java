...
class GameServiceTest {
    ...
    void createTwoPlayerGame() {
        ...
        var game = gameService.create(user0);
        log.info("game created: {}", game);

        var playerJoinedUpdate = (GameUpdate.PlayerJoined) gameService.addPlayer(game, user1);
        game = playerJoinedUpdate.game();
        log.info("player added: {}", playerJoinedUpdate.player());

        var roundStartedUpdate = (GameUpdate.RoundStarted) gameService.startNewRound(game, user0);
        game = roundStartedUpdate.game();
        log.info("round started: {}", roundStartedUpdate.round());
    }
